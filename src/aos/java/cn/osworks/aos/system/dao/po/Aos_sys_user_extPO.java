package cn.osworks.aos.system.dao.po;

import cn.osworks.aos.core.typewrap.PO;

/**
 * <b>用户扩展信息表[aos_sys_user_ext]数据持久化对象</b>
 * <p>
 * 注意:此文件由AOS平台自动生成-禁止手工修改。
 * </p>
 * 
 * @author AHei
 * @date 2016-01-24 16:35:15
 */
public class Aos_sys_user_extPO extends PO {

	private static final long serialVersionUID = 1L;

	/**
	 * 用户ID
	 */
	private String id_;
	
	/**
	 * 电子邮件
	 */
	private String email_;
	
	/**
	 * 固定电话
	 */
	private String fixed_phone_;
	
	/**
	 * 移动电话
	 */
	private String mobile_phone_;
	
	/**
	 * 联系地址
	 */
	private String address_;
	
	/**
	 * 邮编
	 */
	private String zip_;
	
	/**
	 * 生日
	 */
	private String birthday_;
	
	/**
	 * 身份证号
	 */
	private String idno_;
	
	/**
	 * QQ
	 */
	private String qq_;
	
	/**
	 * 动态扩展字段
	 */
	private String dynamic_field_;
	
	/**
	 * 备注
	 */
	private String remark_;
	
	/**
	 * 静态扩展字段1
	 */
	private String filed1_;
	
	/**
	 * 静态扩展字段2
	 */
	private String filed2_;
	
	/**
	 * 静态扩展字段3
	 */
	private String filed3_;
	

	/**
	 * 用户ID
	 * 
	 * @return id_
	 */
	public String getId_() {
		return id_;
	}
	
	/**
	 * 电子邮件
	 * 
	 * @return email_
	 */
	public String getEmail_() {
		return email_;
	}
	
	/**
	 * 固定电话
	 * 
	 * @return fixed_phone_
	 */
	public String getFixed_phone_() {
		return fixed_phone_;
	}
	
	/**
	 * 移动电话
	 * 
	 * @return mobile_phone_
	 */
	public String getMobile_phone_() {
		return mobile_phone_;
	}
	
	/**
	 * 联系地址
	 * 
	 * @return address_
	 */
	public String getAddress_() {
		return address_;
	}
	
	/**
	 * 邮编
	 * 
	 * @return zip_
	 */
	public String getZip_() {
		return zip_;
	}
	
	/**
	 * 生日
	 * 
	 * @return birthday_
	 */
	public String getBirthday_() {
		return birthday_;
	}
	
	/**
	 * 身份证号
	 * 
	 * @return idno_
	 */
	public String getIdno_() {
		return idno_;
	}
	
	/**
	 * QQ
	 * 
	 * @return qq_
	 */
	public String getQq_() {
		return qq_;
	}
	
	/**
	 * 动态扩展字段
	 * 
	 * @return dynamic_field_
	 */
	public String getDynamic_field_() {
		return dynamic_field_;
	}
	
	/**
	 * 备注
	 * 
	 * @return remark_
	 */
	public String getRemark_() {
		return remark_;
	}
	
	/**
	 * 静态扩展字段1
	 * 
	 * @return filed1_
	 */
	public String getFiled1_() {
		return filed1_;
	}
	
	/**
	 * 静态扩展字段2
	 * 
	 * @return filed2_
	 */
	public String getFiled2_() {
		return filed2_;
	}
	
	/**
	 * 静态扩展字段3
	 * 
	 * @return filed3_
	 */
	public String getFiled3_() {
		return filed3_;
	}
	

	/**
	 * 用户ID
	 * 
	 * @param id_
	 */
	public void setId_(String id_) {
		this.id_ = id_;
	}
	
	/**
	 * 电子邮件
	 * 
	 * @param email_
	 */
	public void setEmail_(String email_) {
		this.email_ = email_;
	}
	
	/**
	 * 固定电话
	 * 
	 * @param fixed_phone_
	 */
	public void setFixed_phone_(String fixed_phone_) {
		this.fixed_phone_ = fixed_phone_;
	}
	
	/**
	 * 移动电话
	 * 
	 * @param mobile_phone_
	 */
	public void setMobile_phone_(String mobile_phone_) {
		this.mobile_phone_ = mobile_phone_;
	}
	
	/**
	 * 联系地址
	 * 
	 * @param address_
	 */
	public void setAddress_(String address_) {
		this.address_ = address_;
	}
	
	/**
	 * 邮编
	 * 
	 * @param zip_
	 */
	public void setZip_(String zip_) {
		this.zip_ = zip_;
	}
	
	/**
	 * 生日
	 * 
	 * @param birthday_
	 */
	public void setBirthday_(String birthday_) {
		this.birthday_ = birthday_;
	}
	
	/**
	 * 身份证号
	 * 
	 * @param idno_
	 */
	public void setIdno_(String idno_) {
		this.idno_ = idno_;
	}
	
	/**
	 * QQ
	 * 
	 * @param qq_
	 */
	public void setQq_(String qq_) {
		this.qq_ = qq_;
	}
	
	/**
	 * 动态扩展字段
	 * 
	 * @param dynamic_field_
	 */
	public void setDynamic_field_(String dynamic_field_) {
		this.dynamic_field_ = dynamic_field_;
	}
	
	/**
	 * 备注
	 * 
	 * @param remark_
	 */
	public void setRemark_(String remark_) {
		this.remark_ = remark_;
	}
	
	/**
	 * 静态扩展字段1
	 * 
	 * @param filed1_
	 */
	public void setFiled1_(String filed1_) {
		this.filed1_ = filed1_;
	}
	
	/**
	 * 静态扩展字段2
	 * 
	 * @param filed2_
	 */
	public void setFiled2_(String filed2_) {
		this.filed2_ = filed2_;
	}
	
	/**
	 * 静态扩展字段3
	 * 
	 * @param filed3_
	 */
	public void setFiled3_(String filed3_) {
		this.filed3_ = filed3_;
	}
	

}